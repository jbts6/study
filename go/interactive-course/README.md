...
若 `8080` 已被其他本机程序占用，可把后端改到其他端口，并在启动前端时设置相同的代理目标。例如后端使用 `8081` 时，在 PowerShell 中先执行 `$env:GO_COURSE_API_URL = "http://127.0.0.1:8081"`。

## 课程范围

“Go 起步”共 22 节，按三段推进：

1. 语言基础（01-10）：程序入口、变量、控制流、函数、结构体、指针、方法、接口、错误处理和包配置边界。
