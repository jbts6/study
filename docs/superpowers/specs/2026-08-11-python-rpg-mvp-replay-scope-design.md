...
- 只保留一条已有的端到端战斗测试，直接证明固定输入能够得到正确最终状态。
...
`combat-core.e2e.test.ts` 继续运行同一段固定战斗流程，但不再创建或校验 Replay。测试只断言 `phase === "won"`、`revision === 5` 和敌方 golem 的 `hp === 0`，不再检查每回合事件、冷却、哈希或两次运行快照。

当前没有生产代码调用 Replay，也没有公开 API 或持久化存档依赖这些类型，因此不需要迁移层、兼容读取器或废弃周期。

### 测试收敛

- 随 Replay/JCS 实现删除 13 个专用测试，不保留兼容测试或空壳测试。
- 不为“文件已删除”“依赖已移除”或 Roadmap 文案新增测试。
- 不扩展现有战斗单元测试；本次只修改一条已有端到端测试。
- 验证只运行战斗相关测试与生产构建，不运行无关 Runner 或全项目测试。
