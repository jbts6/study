...
五步命令和初始数值已经在 Task 1 固定，不得在此任务修改数值。每步 rng 均为 2463534242（fixture 无 chancePermille effect）；step 1 至 step 5 的 scout spark cooldown 固定为 [0,0,1,1,0]，其中 step 3 为 1、step 5 为 0；每步 eventsHash 与 stateHash 由 recordAcceptedTurn 的 \`canonicalSha256\` 写入，端到端测试必须断言两次运行的五个 \`rngBefore/rngAfter/eventsHash/stateHash\` 完全相等。step 1 为 move 0,0→1,0、伤害 2、golem hp 6；step 2 额外发出 golem smash 的 cooldown_changed（初始 remainingCooldown 1 降为 0）；step 3 为 spark 伤害 4、golem hp 2；step 5 普攻伤害 2、golem hp 0、spark cooldown 0、phase won，终局不发 turn_advanced。最终事件 stateRevision 全为该步输入 revision+1，seq 连续从 1。
...
- [ ] **Step 5: 提交**
\`\`\`bash
git add rpg/src/game/combat/combat-core.e2e.test.ts docs/superpowers/plans/2026-08-10-python-rpg-combat-core.md
