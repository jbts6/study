...
private createRunRequest(snapshot: GameSnapshot, runId: string): RunRequest {
  ...
  const sourceFile = program.sourceFileName(snapshot.currentLevelId);
  const base = {
  ...
  return program.language === "python"
    ? { ...base, language: "python", entrypoint: { file: sourceFile, callable: "choose_turn" }, allowedModules: ["math"] }
    : { ...base, entrypoint: { file: sourceFile } };
}
```

将 `RUN_LIMITS` 移到控制器依赖或战役运行配置中，使 Go 后续可以提供单独的编译与执行限制；Python 数值维持 `5_000` 毫秒。
...
```bash
git add rpg/src/runners/protocol rpg/src/app/app-controller.ts rpg/src/app/app-controller.test.ts
...
Run: `cd rpg && npx vitest run src/vscode/level-workspace.test.ts src/vscode/workspace-save-store.test.ts src/vscode/game-session.test.ts && npm run typecheck`  
Expected: PASS。
...
it("将 Go 编译错误映射到 strategy.go 的行列", async () => {
  ...
  expect(result).toMatchObject({
    ...
    diagnostics: [{ location: { file: "strategy.go", line: 2 } }],
  });
});
```
...
在 `go-project.ts` 用 `mkdtemp` 创建临时构建目录，写入 `go.mod`、`strategy.go`、`sdk.go`、`runner_main.go`；用参数数组执行 `go build -o <binary> .`，运行二进制时把 `WorldView` JSON 写入 stdin。只缓存由 `strategy.go`、SDK 版本、`go version` 和平台构成的 hash 对应二进制，保存于 `context.globalStorageUri/go-cache/`。清理临时目录始终由该目录创建者负责。

将当前 `PythonRunnerAdapter` 中与语言无关的单次运行状态、超时与中断逻辑迁移为 `SingleRunAdapter`，Python 与 Go 都通过 `LocalProgramProcess` 实现接入；错误文案由各语言运行器提供，不在共享层写 Python 文案。
...
启动器点击任一已注册战役后创建独立 `WorkspaceSaveStore` 与 `DocumentWorkspace`。`package.json` 的 Ctrl/Cmd+Enter 条件改为 `editorLangId == python || editorLangId == go`；新增 `goRpg.goPath` 配置，说明为可选 Go stable 可执行文件路径。不要添加 Rust 配置项。
...
Run: `cd rpg && npx playwright test src/runners/go/e2e.spec.ts`  
Expected: 当 `go version` 可用时，Go 第一关可编辑、运行 `Wait(world)` 并在错误源码中显示编译诊断；工具链缺失时，断言安装提示且跳过真实 Go 执行断言。
