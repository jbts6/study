...
export function validateWorldCommand(
  ...
  if (keys.length !== expectedKeys.length || keys.some((key, index) => key !== expectedKeys[index])) {
    ...
    return { accepted: false, errors: [error(unknown === undefined ? "INVALID_COMMAND" : "UNKNOWN_FIELD", unknown ?? "", unknown === undefined ? "指令字段不完整" : "指令包含未知字段")] };
  }
  ...
  if (type === "travel") {
    ...
    if (!location.connectedLocationIds.includes(input.locationId)) return { accepted: false, errors: [error("TRAVEL_LOCKED", "locationId", "地点未连接或尚未解锁")] };
  ...
  if (type === "talk") {
    if (!location.npcIds.includes(input.targetId) || content.npcs[input.targetId] === undefined) return { accepted: false, errors: [error("INVALID_TARGET", "targetId", "NPC 不在当前地点")] };
    return { accepted: true, command: input as WorldCommand };
  }
  if (type === "inspect") {
  ...
  if (type === "collect") {
    if (!location.itemSourceIds.includes(input.targetId) || content.itemSources[input.targetId] === undefined) return { accepted: false, errors: [error("INVALID_TARGET", "targetId", "材料来源不在当前地点")] };
